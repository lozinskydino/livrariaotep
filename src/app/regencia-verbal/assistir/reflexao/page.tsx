...
export default function ReflexaoAssistirPage() {
  const router = useRouter();

  return (
    <div className="min-h-dvh bg-[#F2F6F9]">
      <div
        className="mx-auto w-full max-w-[393px] px-4 sm:px-0 py-4"
        style={{ paddingTop: "max(16px, env(safe-area-inset-top))", paddingBottom: "max(24px, env(safe-area-inset-bottom))" }}
      >
        {/* Container com o mesmo visual do modal */}
        <div className="relative bg-[#E6F2FB] rounded-[16px] rounded-tl-[28px] rounded-tr-[12px] shadow-[0_12px_32px_rgba(0,0,0,0.2)] overflow-hidden">
          ...
          <button
            onClick={() => router.back()}
            className="absolute top-4 left-4 inline-flex items-center gap-2 bg-[#2B7BDE] text-white rounded-full pl-3 pr-3 py-[6px] shadow-[0_6px_14px_rgba(43,123,222,0.35)] hover:bg-[#216bc5] transition"
            aria-label="Voltar"
          >
          ...
          <div className="px-6 pt-10 pb-2 text-center">
            <h1 className={`${inter.className} text-[26px] leading-[30px] font-semibold text-[#2B7BDE]`}>Assistir</h1>
          ...
          <div className="px-6 py-6 space-y-5">
            {/* Linha 1 */}
            <div className="grid grid-cols-[188px_126px] gap-4 items-center">
              <div className="flex flex-row justify-start items-center px-[14px] gap-[10px] w-[188px] h-[145px] bg-[#3A7BD5] text-white rounded-[20px] shadow-[0_6px_16px_rgba(43,123,222,0.25)]">
                <p
